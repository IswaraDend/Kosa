...
const GudangPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editing) {
        await api.put(`/super-admin/warehouses/${editing.id}`, form);
      } else {
        await api.post('/super-admin/warehouses', { ...form, project_id: Number(selectedProjectId) });
      }
      setFormOpen(false);
      loadWarehouses();
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/super-admin/warehouses/${confirmDelete.id}`);
